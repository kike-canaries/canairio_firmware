...
void M5TFTUtils::welcomeAddMessage(String msg) {
...
void M5TFTUtils::welcomeRepeatMessage(String msg) {
...
void M5TFTUtils::showWindowBike(){
    showStatus();
...
void M5TFTUtils::showInfoWindow() {
    showStatus();
    tft.setTextColor(TFT_GREENYELLOW, TFT_BLACK);
    tft.setTextFont(1);
    tft.setFreeFont(&Orbitron_Medium_20);
...
void M5TFTUtils::refreshInfoWindow() {
    if (state != 7) return;
    tft.setTextColor(TFT_WHITE, TFT_BLACK);
    tft.setTextFont(2);
...
void M5TFTUtils::loadBrightness() {
    for (int i = 0; i < 5; i++) {
...
void M5TFTUtils::updateBrightness() {
...
void M5TFTUtils::updateInvertValue(){
...
void M5TFTUtils::updateBatteryValue(){
...
uint8_t M5TFTUtils::getBatteryLevel(){
    float volts = battGetVoltage();
    return battCalcPercentage(volts); 
}

float M5TFTUtils::getBatteryVoltage(){
    return battGetVoltage();
}

void M5TFTUtils::setWifiMode(bool enable){
    _wifi_enable = enable;
    updateWifiMode();
}

void M5TFTUtils::setPaxMode(bool enable){
    _pax_enable = enable;
    updateWifiMode();
}

void M5TFTUtils::notifyWifiMode(){
...
void M5TFTUtils::updateWifiMode(){
    if (state < 1) return;
...
void M5TFTUtils::notifySampleTime(){
    if(++st>=5) st = 0;
...
void M5TFTUtils::setSampleTime(int time){
    _sample_time = time;
    updateSampleTime();
}

void M5TFTUtils::updateSampleTime() {
    if (state < 1) return;
...
void M5TFTUtils::updateCalibrationField(){
...
void M5TFTUtils::showProgress(unsigned int progress, unsigned int total) {
    suspendTaskGUI();
...
void M5TFTUtils::displayCenterBig(String msg) {
    tft.setFreeFont(&Orbitron_Light_32);
    tft.setTextDatum(MC_DATUM);
    tft.setTextColor(TFT_WHITE,TFT_BLACK);
...
void M5TFTUtils::displayMainUnit(String unit) {
...
void M5TFTUtils::displayBottomLine(String msg) {
    tft.setTextFont(1);
...
void M5TFTUtils::displayEmoticonLabel(int cursor, String msg) {
    
}

void M5TFTUtils::displayEmoticonColor(uint32_t color, String msg) {
...
void M5TFTUtils::displayBigEmoticon(String msg) {

}

void M5TFTUtils::displayBigLabel(int cursor, String msg) {

}

void M5TFTUtils::displaySensorAverage(int average) {
    int color =getAQIColor(average);
    displayEmoticonColor(aqicolors[color], aqilabels[color]);
}

void M5TFTUtils::displayMainValues(){
...
void M5TFTUtils::setSensorData(int mainValue, float humi, float temp, int rssi, int deviceType) {
    suspendTaskGUI();
...
void M5TFTUtils::setGUIStatusFlags(bool wifiOn, bool bleOn, bool blePair) {
...
void M5TFTUtils::setInfoData(String info) {
    suspendTaskGUI();
    _info = info;
    resumeTaskGUI();
}

void M5TFTUtils::displayGUIStatusFlags() {
...
uint32_t M5TFTUtils::getAQIColor(uint32_t value) {
    if (_deviceType <= 1) {
...
double M5TFTUtils::getMultiplicator() {
  uint32_t maxVal = 1;
  for (int i = 0; i < MAX_X; i++) {
...
void M5TFTUtils::displayDataOnIcon() {
    dataOn = true;
}

/// enable trigger for sensor live icon, one time.
void M5TFTUtils::displaySensorLiveIcon() {
    sensorLive = true;
    _live_ticks = 0;
}

/// enable trigger for save preference ok, one time.
void M5TFTUtils::displayPreferenceSaveIcon() {
    preferenceSave = true;
}

void M5TFTUtils::drawBluetoothIcon () {
    tft.drawBitmap(122, 1, iconBluetoothPaired, 12, 16, TFT_BLACK);
}

void M5TFTUtils::drawWifiHighIcon () {
    tft.drawBitmap(109, 1, iconWifiHigh, 12, 16, TFT_BLACK);
}

void M5TFTUtils::drawWifiMidIcon () {
    tft.drawBitmap(109, 1, iconWifiMid, 12, 16, TFT_BLACK);
}

void M5TFTUtils::drawWifiLowIcon () {
    tft.drawBitmap(109, 1, iconWifiLow, 12, 16, TFT_BLACK);
}

void M5TFTUtils::drawFanIcon () {
    tft.drawBitmap(0, 1, fanState ? iconFanState0 : iconFanState1, 12, 16, TFT_BLACK);
    fanState = !fanState;
}

void M5TFTUtils::drawDataIcon () {
    tft.drawBitmap(96, 1, iconArrows, 12, 16, TFT_BLACK);
}

void M5TFTUtils::drawPreferenceSaveIcon () {
    tft.drawBitmap(83, 1, iconSave, 12, 16, TFT_BLACK);
}

void M5TFTUtils::pageStart() {
...
void M5TFTUtils::pageEnd() {

}

void M5TFTUtils::clearScreen() {
    tft.fillScreen(TFT_BLACK);
}

void M5TFTUtils::setBrightness(uint32_t value) {
    brightness = value;
}

void M5TFTUtils::notifyBrightness() {
    // M5.Lcd.setBrightness((uint8_t) brightness);
}

void M5TFTUtils::setCallbacks(GUIUserPreferencesCallbacks* pCallBacks){
    mGUICallBacks = pCallBacks;
}

void M5TFTUtils::setTrackValues(float speed, float distance){
    suspendTaskGUI();
    _speed = speed;
    _km = distance;
    resumeTaskGUI();
...
void M5TFTUtils::setTrackTime(int h, int m, int s){
    suspendTaskGUI();
    _hours = h;
    _minutes = m;
    _seconds = s;
...
void M5TFTUtils::suspendTaskGUI(){
    if(taskGUIrunning) vTaskSuspend(xHandle);
}

void M5TFTUtils::resumeTaskGUI(){
    if(taskGUIrunning) vTaskResume(xHandle);
}

M5TFTUtils* M5TFTUtils::getInstance() {
	return this;
}

void M5TFTUtils::loop(){
    // OLED GUI compatibility
}

/// Firmware version from platformio.ini
String M5TFTUtils::getFirmwareVersionCode() {
